...
interface CachedVideoGridProps {
    filter: (video: VideoDto) => boolean;
    username: string;
}

const CachedVideoGrid = ({filter, username}: CachedVideoGridProps) => {
    const {t} = useTranslation();
    const navigate = useNavigate();
    const allVideos = useAppSelector((s) => s.videosCache.videos);

    const videos = useMemo(
        () => Object.values(allVideos).filter(filter),
        [allVideos, filter]
    );
    ...
    return (
        <div className="h-full w-full overflow-y-auto px-4 pb-8">
            <div
...
export default CachedVideoGrid;
